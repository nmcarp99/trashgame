<?php
if (file('txt/'.$_COOKIE['game'].'turn.txt')[0] == $_COOKIE['player'] and file('txt/'.$_COOKIE['game'].'discard.txt')[0] != '') {
	if (file('txt/'.$_COOKIE['game'].'discard.txt')[0][0] == $_GET['card'][0] and $GET['card'] != 1 or (file('txt/'.$_COOKIE['game'].'discard.txt')[0][0] == 'A' and $_GET['card'] == 1) or file('txt/'.$_COOKIE['game'].'discard.txt')[0][0] == 'K') {
		...
		foreach($useCardsFile as $card) {
			$line = $line + 1;
			if ($_GET['card'] == $line) {
				fwrite($myfile, file('txt/'.$_COOKIE['game'].'discard.txt')[0]);
			}
			else {
				fwrite($myfile, $card);
			}
		...
		fclose($myfile);
		$discardCards = file('txt/'.$_COOKIE['game'].'discard.txt');
		unset($discardCards[0]);
